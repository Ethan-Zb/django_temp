...
from helper import helper_int, helper_string
...
class LibRedis:
    ...
    def set_p_expire(self, keyname=None, milliseconds=0):
        ...
        @return:
        """
        milliseconds = helper_int.parse_to_int(milliseconds)
    ...
    @wraps_set_expire
    def hIncrbyInt(self, keyname=None, key=None, value=0):
        """
        为哈希表 key 中的域 field 的值加上增量 increment 。field 的值必须为int
        增量也可以为负数，相当于对给定域进行减法操作。
        如果 key 不存在，一个新的哈希表被创建并执行 HINCRBY 命令。
        如果域 field 不存在，那么在执行命令前，域的值被初始化为 0 。
        @param keyname: hash name
        @param key:
        @param value: int 正负值
        ...
        keyname = self.key_make(keyname.strip())
        key = helper_string.to_string(key)
        value = helper_int.parse_to_int(value)
        if not all([keyname, key, value]):
            return 0
        return self.obj_redis.hincrby(name=keyname, key=key, amount=value)
    
    @wraps_set_expire
    def hIncrbyFloat(self, keyname=None, key=None, value=0):
        """
        为哈希表 key 中的域 field 的值加上增量 increment 。field 的值必须为 Float
        增量也可以为负数，相当于对给定域进行减法操作。
        如果 key 不存在，一个新的哈希表被创建并执行 HINCRBY 命令。
        如果域 field 不存在，那么在执行命令前，域的值被初始化为 0 。
        @param keyname: hash name
        @param key:
        @param value: float 正负值
        @return: 执行 HINCRBY 命令之后，哈希表 key 中域 field 的值。
        """
        if not keyname:
            return 0
        ...
        if not all([keyname, key, value]):
            return 0
        return self.obj_redis.hincrbyfloat(name=keyname, key=key, amount=value)
